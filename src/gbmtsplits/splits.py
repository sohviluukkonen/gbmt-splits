...
class GloballyBalancedSplit:
    ...
    def __init__(
            ...
            clustering_method : Callable = MaxMinClustering(),
            n_splits : int = 1,
    ...
    def __call__(
        ...
        if self.n_splits > 1 and not (isinstance(self.clustering_method, MaxMinClustering)
                                      or isinstance(self.clustering_method, RandomClustering)):
            raise ValueError(f'n_splits > 1 does not work with type {type(self.clustering_method).__name__}.')
        ...
        data.drop(self.targets_for_balancing, axis=1, inplace=True)
